...
//                    }
                ...
                //将参数传给tab页的param属性，此时tab页内的内容不重新加载		2011-08-16 evan.ban
                preParamOverrideTabItem : function(targettabid,param){
                    var tabitem = $("li[tabid="+targettabid+"]", g.tab.links.ul); 
                    var contentitem = $(".l-tab-content-item[tabid="+targettabid+"]",g.tab.content);
                    $("iframe", contentitem).attr("param", param);
                },
                //将url传给tab页的preSrc属性，此时tab页内的内容不重新加载
                preOverrideTabItem : function(targettabid,url){
                    var tabitem = $("li[tabid="+targettabid+"]", g.tab.links.ul); 
                    var contentitem = $(".l-tab-content-item[tabid="+targettabid+"]",g.tab.content);
                    $("iframe", contentitem).attr("preSrc", url);
                },
                //改变标签页的URL，其他属性不变
                overrideTabUrl : function(targettabid,url){
                    var tabitem = $("li[tabid="+targettabid+"]", g.tab.links.ul); 
                    var contentitem = $(".l-tab-content-item[tabid="+targettabid+"]",g.tab.content);
                    $("iframe", contentitem).attr("src", url);
                },
                //选中tab页时，此时tab页内的内容重新加载		2011-08-16 evan.ban
                realOverrideTabItem : function(targettabid){
                   frameLoad(targettabid,p,g);
                },
                //仅保留一个tab		2011-08-16 evan.ban
...
//                    	alert(e);
			...
			try{
				if(p.menuTab!=null){   
		            $(".l-tab-links ul").after("<div class='shortRunBtn' style='border:0px'></div>"); 
		            $(".l-tab-links .shortRunBtn").ligerMenuBar(p.menuTab).css("width",navigator.userAgent.indexOf("MSIE")!=-1?"157px":"157px");
		            $(".l-tab-links li:first").each(function(i){
						$(this).css("margin-left",navigator.userAgent.indexOf("MSIE")!=-1?"160px":"160px");
		            });
	            }
            }catch(e){
            	alert(e);
...
function frameLoad(targettabid,p,g){
    var tabitem = $("li[tabid="+targettabid+"]", g.tab.links.ul); 
    var contentitem = $(".l-tab-content-item[tabid="+targettabid+"]",g.tab.content);
    var finalSrc=$("iframe", contentitem).attr("preSrc");
    var paramSrc=$("iframe", contentitem).attr("param");
    var hisSrc=$("iframe", contentitem).attr("src");
    if(paramSrc!=null && paramSrc!=undefined )
    	finalSrc+=paramSrc;
    if($("iframe", contentitem).attr("finalSrc")!="" &&( hisSrc!=finalSrc || p.reloadWhenSelect==true))
    	$("iframe", contentitem).attr("src",finalSrc);
}