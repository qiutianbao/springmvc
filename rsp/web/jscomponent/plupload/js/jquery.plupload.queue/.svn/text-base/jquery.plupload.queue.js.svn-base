		...
		if (e) {
			this.each(function() {
				...
				function h(l) {
					...
					var m = c("#" + l.id).attr("class", n).find("a").css(
							"display", "block");
					if (l.hint) {
						m.attr("title", l.hint)
					}
...
//							.text(a("Uploaded %d/%d files").replace(/%d\/%d/,
				...
				function g() {
					...
					c.each(j.files, function(p, o) {
						...
						if (o.status == plupload.DONE) {
							if (o.target_name) {
								l += '<input type="hidden" name="' + k + "_"
										+ n + '_tmpname" value="'
										+ o.rid
										+ '" />'
							}
							l += '<input type="hidden" name="' + k + "_" + n
									+ '_name" value="'
					...
					c("span.plupload_total_file_size", i).html(plupload
							.formatSize(j.total.size));
					if (j.total.queued === 0) {
						c("span.plupload_add_text", i).text(a("添加文件"))
					} else {
						c("span.plupload_add_text", i).text(j.total.queued
								+ " 文件队列")
					}
					...
					if (!j.files.length && j.features.dragdrop
							&& j.settings.dragdrop) {
						c("#" + k + "_filelist")
								.append('<li class="plupload_droptext">'
										+ a("Drag files here.") + "</li>")
					}
				}
				...
				j.bind("Error", function(l, o) {
					...
					if (m) {
						...
						if (o.code == plupload.FILE_SIZE_ERROR) {
							alert(a("Error: 上传文件太大: ") + m.name) ;
						}
						if (o.code == plupload.FILE_EXTENSION_ERROR) {
							alert(a("Error: 无效的文件扩展名: ") + m.name) ;
						}
						if (o.code == '10002') {
							alert(a("Error: 上传失败: ") + m.name) ;
						}
						m.hint = n;
						c("#" + m.id).attr("class", "plupload_failed")
								.find("a").css("display", "block").attr(
										"title", n);
					}
				});
...
//								.hide();
						...
						c(
								...
								i).css("display", "block");
//						c("span.plupload_upload_status", i).text("Uploaded "
//								+ j.total.uploaded + "/" + j.files.length
//								+ " files");
				...
				j.bind("UploadProgress", function(l, m) {
					...
					f();
					if (e.multiple_queues
							&& j.total.uploaded + j.total.failed == j.files.length) {
						c(".plupload_buttons,.plupload_upload_status", i).css(
								"display", "inline");
