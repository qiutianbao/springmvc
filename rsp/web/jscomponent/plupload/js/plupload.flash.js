	...
	d.runtimes.Flash = d.addRuntime("flash", {
		...
		init : function(j, o) {
			...
			d.extend(i.style, {
						position : "absolute",
						top : "0px",
						background : j.settings.shim_bgcolor || "transparent",
						zIndex : 99999,
						width : "100%",
						height : "100%"
					});
			...
			l();
			n = i = null;
			j.bind("Flash:Init", function() {
				var r = {}, q;
				m().setFileFilters(j.settings.filters,
						j.settings.multi_selection);
				if (g[j.id]) {
					return
				}
				g[j.id] = true;
				j.bind("UploadFile", function(s, u) {
							var v = s.settings, t = j.settings.resize || {};
							m().uploadFile(r[u.id], v.url, {
								name : u.target_name || u.name,
								mime : d.mimeTypes[u.name.replace(
										/^.+\.([^.]+)/, "$1").toLowerCase()]
				...
				j.bind("Flash:UploadProcess", function(t, s) {
				...
				j.bind("Flash:UploadChunkComplete", function(s, u) {
				...
				j.bind("Flash:SelectFiles", function(s, v) {
							var u, t, w = [], x;
							...
							if (w.length) {
								j.trigger("FilesAdded", w)
							}
						});
				j.bind("Flash:SecurityError", function(s, t) {
							j.trigger("Error", {
										code : d.SECURITY_ERROR,
										message : d
												.translate("Security error."),
										details : t.message,
										file : j.getFile(r[t.id])
									})
						});
				j.bind("Flash:GenericError", function(s, t) {
							j.trigger("Error", {
				...
				j.bind("Flash:IOError", function(s, t) {
							j.trigger("Error", {
										code : d.IO_ERROR,
				...
				j.bind("Flash:ImageError", function(s, t) {
							j.trigger("Error", {
				...
				j.bind("Flash:StageEvent:rollOver", function(s) {
							var t, u;
							t = b.getElementById(j.settings.browse_button);
							u = s.settings.browse_button_hover;
							if (t && u) {
								d.addClass(t, u)
							}
						});
				j.bind("Flash:StageEvent:rollOut", function(s) {
							var t, u;
							t = b.getElementById(j.settings.browse_button);
							u = s.settings.browse_button_hover;
							if (t && u) {
				...
				j.bind("Flash:StageEvent:mouseDown", function(s) {
							var t, u;
							t = b.getElementById(j.settings.browse_button);
				...
				j.bind("Flash:StageEvent:mouseUp", function(s) {
							var t, u;
							t = b.getElementById(j.settings.browse_button);
							u = s.settings.browse_button_active;
							if (t && u) {
				...
				j.bind("QueueChanged", function(s) {
							j.refresh()
						});
				j.bind("FilesRemoved", function(s, u) {
							var t;
							for (t = 0; t < u.length; t++) {
								m().removeFile(r[u[t].id])
							}
						});
				j.bind("StateChanged", function(s) {
							j.refresh()
						});
				j.bind("Refresh", function(s) {
					var t, u, v;
					m().setFileFilters(j.settings.filters,
							j.settings.multi_selection);
