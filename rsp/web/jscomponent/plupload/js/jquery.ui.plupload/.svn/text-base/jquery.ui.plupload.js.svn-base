(function(f, a, c, g, e) {
	...
	g.widget("ui.plupload", {
		...
		_create : function() {
			...
			d(this.element);
			this.container = g(".plupload_container", this.element).attr("id",
					k + "_container");
			this.filelist = g(".plupload_filelist_content", this.container)
					.attr("id", k + "_filelist");
			this.browse_button = g(".plupload_add", this.container).attr("id",
					k + "_browse");
			this.start_button = g(".plupload_start", this.container).attr("id",
					k + "_start");
			this.stop_button = g(".plupload_stop", this.container).attr("id",
					k + "_stop");
			if (g.ui.button) {
...
//										+ (i.runtime = m.runtime));
			...
			j.bind("UploadProgress", function(l, m) {
						g("#" + m.id + " .plupload_file_status", i.element)
								.html(m.percent + "%");
						i._handleFileStatus(m);
					...
					if (m) {
						...
						switch (p.code) {
							...
							case c.FILE_SIZE_ERROR :
								m = b("File: %f, size: %s, max file size: %m")
										.replace(/%([fsm])/g, function(r, q) {
											switch (q) {
												case "f" :
													return n.name;
												case "s" :
							...
							case i.FILE_COUNT_ERROR :
								m = b("Upload element accepts only %d file(s) at a time. Extra files were stripped.")
										.replace("%d", i.options.max_file_count);
								break;
							...
							case c.IMAGE_DIMENSIONS_ERROR :
								m = c
										.translate("Resoultion out of boundaries! <b>%s</b> runtime supports images only up to %wx%hpx.")
										.replace(/%([swh])/g, function(r, q) {
													switch (q) {
														case "s" :
															return l.runtime;
														case "w" :
		...
		_handleState : function() {
			...
			if (j.state === c.STARTED) {
				...
				g([]).add(i.stop_button).add(".plupload_started")
						.removeClass("plupload_hidden");
//				g(".plupload_upload_status", i.element)
//						.text(b("Uploaded %d/%d files").replace("%d/%d",
//								j.total.uploaded + "/" + j.files.length));
				...
				if (i.options.multiple_queues) {
					...
					g(".plupload_header_content", i.element)
							.removeClass("plupload_header_content_bw")
				}
				i._updateFileList()
			}
		},
		...
		_updateTotalProgress : function() {
			...
			g(".plupload_total_status", this.element).html(i.total.percent
					+ "%");
//			g(".plupload_upload_status", this.element)
//					.text(b("Uploaded %d/%d files").replace("%d/%d",
//							i.total.uploaded + "/" + i.files.length))
		},
		_updateFileList : function() {
			...
			g.each(n.files, function(q, p) {
				...
				if (p.status === c.DONE) {
					if (p.target_name) {
						i += '<input type="hidden" name="' + o
								+ '_tmpname" value="'
								+ c.xmlEncode(p.target_name) + '" />'
					}
					i += '<input type="hidden" name="' + o + '_name" value="'
							+ c.xmlEncode(p.name) + '" />';
					i += '<input type="hidden" name="' + o + '_status" value="'
							+ (p.status === c.DONE ? "done" : "failed")
							+ '" />';
					k++;
					j.counter.val(k)
				}
				...
				g("#" + p.id + ".plupload_delete .ui-icon, #" + p.id
						+ ".plupload_done .ui-icon").click(function(r) {
							g("#" + p.id).remove();
							n.removeFile(p);
							r.preventDefault()
						});
			...
			g(".plupload_total_file_size", j.element).html(c
					.formatSize(n.total.size));
			if (n.total.queued === 0) {
				g(".ui-button-text", j.browse_button).text(b("Add Files"))
			} else {
				g(".ui-button-text", j.browse_button).text(b("%d files queued")
						.replace("%d", n.total.queued))
			}
		...
		_enableRenaming : function() {
			var i = this;
			g(".plupload_file_name span", this.filelist).live("click",
					function(o) {
						...
						m
								.hide()
								.after('<input class="plupload_file_rename" type="text" />');
		...
		_enableDragAndDrop : function() {
			this.filelist.append('<tr><td class="plupload_droptext">'
					+ b("Drag files here.") + "</td></tr>");
		...
		_enableSortingList : function() {
			...
			g("tbody", this.filelist).sortable({
				...
				helper : function(l, k) {
					return k.clone(true).find("td:not(.plupload_file_name)")
							.remove().end().css("width", "100%")
				},
				start : function(l, k) {
					j = g("tr", this).index(k.item)
				},
				stop : function(q, p) {
					var l, o, k, n = [], m = g("tr", this).index(p.item);
					for (l = 0, o = i.uploader.files.length; l < o; l++) {
						if (l === m) {
							k = j
						} else {
							if (l === j) {
					...
					n.unshift(n.length);
					n.unshift(0);
					Array.prototype.splice.apply(i.uploader.files, n)
				}
			})
		},
		_notify : function(j, k) {
			var i = g('<div class="plupload_message"><span class="plupload_message_close ui-icon ui-icon-circle-close" title="'
					...
					+ k + "</p></div>");
			i.addClass("ui-state-" + (j === "error" ? "error" : "highlight"))
					.find("p .ui-icon").addClass("ui-icon-"
							+ (j === "error" ? "alert" : "info")).end()
					.find(".plupload_message_close").click(function() {
								i.remove()
							}).end().appendTo(".plupload_header_content",
							this.container)
		},
		destroy : function() {
			g(".plupload_button", this.element).unbind();
			if (g.ui.button) {
				g(".plupload_add, .plupload_start, .plupload_stop",
